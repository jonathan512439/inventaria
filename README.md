...
Migraciones aplicadas: `migrations.sql` (001) → `002_lote_defaults.sql` → `003_tipos_de_producto.sql` → `004_profiles_insert.sql` → `005_ai_usage.sql` → `006_stock_movements.sql` → `007_variantes.sql` (ejes por categoría, variantes con stock propio y disparadores que mantienen el stock del producto = suma de variantes) → `008_product_summaries.sql` (vista ligera del inventario) → `009_ai_keys.sql` (clave de IA propia cifrada; `KEY_ENCRYPTION_SECRET` en Cloudflare) → `010_control_stock.sql` (mínimos, vencimientos, papelera, proveedores/compras, conteos, historial de precios) → `011_avisos.sql` (ajustes de avisos por negocio, categoría y producto) → `012_ventas.sql` (tickets, líneas, caja y cierres) → `013_clientes.sql` (clientes, abonos, entregas en consignación) → `014_negocios.sql` … `018_pin_privado.sql` (negocio como unidad de datos, RLS por negocio, equipo, invitaciones, PIN privado, firma por cabecera `x-actor`) → `019_respaldos.sql` (bucket privado `backups`). Pruebas: `npm run test:team`, `npm run test:import`, `npm run test:backup`. Respaldo semanal: `.github/workflows/backup.yml` ejecuta `npm run backup` (necesita el secreto `SUPABASE_SERVICE_ROLE_KEY` en GitHub, además de `NEXT_PUBLIC_SUPABASE_URL`). Pruebas: `npm run test:e2e` (flujo completo, consume IA) y `npm run test:variants` (reglas de variantes en la base de datos, sin IA).
